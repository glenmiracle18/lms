...
import {
  Plan, Subscription, WebhookEvent
} from "@prisma/client";
...
} from "@/lib/types/webhook_types";

import { configureLemonSqueezy } from "@/lib/lemon_squeezy";
import { webhookHasData, webhookHasMeta } from "@/lib/typeguards";
import { takeUniqueOrThrow } from "@/lib/utils";
import { auth, currentUser } from "@clerk/nextjs";
import { clerkClient } from "@clerk/nextjs/client";
...
export async function getCheckoutURL(variantId: number, embed = false) {
  ...
  // getting the signed in user information
  const { userId } = auth()
...
export async function syncPlans() {
  ...
  const productVariants: NewPlan[] = await db.plan.findMany();
  ...
  async function _addVariant(variant: NewPlan) {
    ...
    const existingVariant = await db.plan.findUnique({
        where: {
      ...
      // Log a message indicating successful sync
      console.log(`${variant.name} synced with the database...`);

      // Push the variant to some array, if needed
...
export async function storeWebhookEvent(
  ...
  const returnedValue = await db.webhookEvent.create({
      data: {
        id, // Assuming `id` is defined somewhere
        eventName,
        processed: false,
        body,
      },
      // If the id already exists, do nothing (equivalent to ON CONFLICT DO NOTHING)
      // Adjust the condition according to your requirement
      where: {
        id: { not: id },
      },
...
export async function processWebhookEvent(webhookEvent: NewWebhookEvent) {
  ...
  const dbWebhookEvent = await prisma.webhookEvent.findUnique({
      where: {
        id: webhookEvent.id, // Assuming `webhookEvent.id` contains the id of the webhook event you want to fetch
      },
    });
  ...
  if (!webhookHasMeta(eventBody)) {
    ...
    if (webhookEvent.eventName.startsWith("subscription_payment_")) {
      ...
      const plan = await prisma.plan.findFirst({
          where: {
            variantId: parseInt(variantId, 10), // Assuming `variantId` is a variable containing the variantId
          },
        });
    ...
    await prisma.webhookEvent.update({
        where: {
          id: webhookEvent.id, // Assuming `webhookEvent.id` contains the id of the webhook event to update
        },
        data: updateData,
      });
}
